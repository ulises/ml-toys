...
(defn adjust-weights [ node example t & update-labels ]
  ...
  (let [node-weights (:weights node)
        ...
        updated-weights (two-d/add-points node-weights
                                          (first (two-d/scalar-product (* (som-core/learning-rate t) (som-core/theta weight-distance node example t))
                                                                       (two-d/substract-points example-weights node-weights))))
        updated-labels (merge-with + (:labels node) {(:label example) 1})
        {x :x y :y} (:point node)
        new-updated-node (som-core/som-node x y updated-weights (:labels node))]
    (if update-labels
  ...
  (let [som-document (:weights node)
        ...
        updated-weights (tokenise/normalise
                         (merge-with + som-document
                                     (reduce merge {}
                                             (map (fn [[k v]] [k (* (som-core/learning-rate t) (som-core/theta weight-distance node example t) v)]) (merge-with (comp util/abs -) example-document som-document)))))
        {x :x y :y} (:point node)
        updated-labels (merge-with (:labels node) {(:label example) 1})
        updated-node (som-core/som-node x y updated-weights (:labels node))]
    (if update-labels
